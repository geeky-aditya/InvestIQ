...
app.post("/newOrder", async (req, res) => {
  try {
    ...
    // 2. Check if stock exists in holdings
    ...
    if (mode === "BUY") {
      if (existingStock) {
        ...
        const newAvg =
          ((existingStock.qty * existingStock.avg) +
            (qty * price)) / totalQty;

        existingStock.qty = totalQty;
        existingStock.avg = newAvg;
        existingStock.price = price; // latest market price
    ...
    // ================= SELL =================
      ...
      // If all shares sold → delete holding
